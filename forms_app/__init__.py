        ...
        if u is None:
            if passc!=password:
                ...
                query='''CREATE TABLE '''+username+'''( 
                          ...
                          maxsub INT)'''
                cur.execute(query)
                query='''CREATE TABLE '''+username+'''_responses(
                            resno INT,
                            formno INT,
                            quesno INT,
                            question VARCHAR(200),
...
def home():
    ...
    cur=conn.cursor()
    msg=None
    q='''SELECT MAX(formno) FROM '''+session['user']
    cur.execute(q)
    nf=cur.fetchone()
    global noti
    q='''SELECT DISTINCT formno , title FROM '''+session['user']
    cur.execute(q)
    data=cur.fetchall()
...
def createform():
    ...
    cur=conn.cursor()
    msg=None
    query='''SELECT MAX(formno) FROM '''+session['user']
    cur.execute(query)
    data=cur.fetchone()
    if data[0] is None:
        formno=1
        ...
        for i in range(len(questions)):
            ...
            if answertype[i]=="checkbox" or answertype[i]=="radio":
                ...
                for j in range(op,op+n):
                    query='''INSERT INTO '''+session['user']+''' VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'''
                    cur.execute(query, 
                                (title,des,formno,questions[i],answertype[i],options[j],require[i],i+1,dl,limit))
                    conn.commit()
            ...
            query='''INSERT INTO '''+session['user']+''' VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'''
            cur.execute(query, 
                             (title,des,formno,questions[i],answertype[i],0,require[i],i+1,dl,limit)) 
            conn.commit() 
...
def view(formno):
    ...
    msg=None
    user=session['user']
    q='''SELECT * FROM '''+session['user']+''' WHERE formno=%s'''
    cur.execute(q,(formno))
    data=cur.fetchall()
    q2='''SELECT MAX(quesno) FROM '''+session['user']+''' WHERE formno=%s'''
    cur.execute(q2,(formno))
    nq=cur.fetchone()
...
def response(user,formno):
    ...
    cur.execute(q2,(formno))
    nq=cur.fetchone()
    q2='''SELECT DISTINCT deadline FROM '''+session['user']+''' WHERE formno=%s'''
    cur.execute(q2,(formno))
    dt=cur.fetchone()
    ...
    if request.method=='POST':
        ...
        noti.append(m)
        a=1
        query='''SELECT * FROM '''+session['user']+''' WHERE formno=%s'''
        cur.execute(query,(formno))
        data=cur.fetchall()
        query='''SELECT MAX(resno) FROM '''+session['user']+'''_responses WHERE formno=%s'''
        cur.execute(query,(formno))
        rn=cur.fetchone()
        if rn[0] is None:
            cur_rn=1
        ...
        query='''SELECT DISTINCT maxsub FROM '''+session['user']+''' WHERE formno=%s'''
        cur.execute(query,(formno))
        l=cur.fetchone()
        ...
        if len(email):
            q='''SELECT COUNT(answer) FROM '''+session['user']+'''_responses WHERE formno=%s AND 
                                                                    answer=%s AND answertype=%s'''
            cur.execute(q,(formno,email[0],'email'))
            sub=cur.fetchone()

        op=0
        if sub is None or sub[0] < l[0]:
            
            query='''INSERT INTO '''+session['user']+'''_responses VALUES (%s,%s,%s,%s,%s,%s)'''
            ...
            for i in range(nq[0]):
                ...
                if data[i][4]=="image" or data[i][4]=="file":
                    ...
                    if f:
                        ...
                        cur.execute(query,(cur_rn,formno,data[i][7],data[i][3],data[i][4],path))
                        conn.commit()

                    else:
                        cur.execute(query,(cur_rn,formno,data[i][7],data[i][3],data[i][4],"NA"))
                        conn.commit()
                ...
                for j in range(len(cur_ans)):
                    if cur_ans[j]:
                        ...
                        cur.execute(query,(cur_rn,formno,data[k][7],data[k][3],data[k][4],cur_ans[j]))
                        conn.commit()
                    else:
                        cur.execute(query,(cur_rn,formno,data[k][7],data[k][3],data[k][4],"NA"))
                        conn.commit()
...
def responses(formno):
    ...
    cq={}
    rq={}
    query='''SELECT * FROM '''+session['user']+'''_responses WHERE formno=%s'''
    cur.execute(query,(formno))
    data=cur.fetchall()
    query='''SELECT * FROM '''+session['user']+''' WHERE formno=%s AND answertype=%s '''
    cur.execute(query,(formno,'checkbox'))
    ...
    if checkdata is None:
        ...
        query='''SELECT * FROM '''+session['user']+'''_responses WHERE formno=%s AND answertype=%s '''
        cur.execute(query,(formno,'checkbox'))
        ans=cur.fetchall()
        for a in ans:
            if a[5] is not None:
    ...
    query='''SELECT * FROM '''+session['user']+''' WHERE formno=%s AND answertype=%s '''
    cur.execute(query,(formno,'radio'))
...
def explore():
    ...
    for d in data:
        query='''SELECT MAX(resno) FROM '''+d[0]+'''_responses'''
        cur.execute(query)
        m=cur.fetchone()
        if m[0] is not None:
            q2='''SELECT DISTINCT formno FROM '''+d[0]+'''_responses WHERE resno=%s'''
            cur.execute(q2,int(m[0]))
            f=cur.fetchone()
...
def expform(user,formno):
    ...
    cur=conn.cursor()
    msg=None
    q='''SELECT * FROM '''+user+''' WHERE formno=%s'''
    cur.execute(q,(formno))
    data=cur.fetchall()
    q2='''SELECT MAX(quesno) FROM '''+user+''' WHERE formno=%s'''
    cur.execute(q2,(formno))
    nq=cur.fetchone()
    return render_template("view.html",data=data,nques=nq[0],user=user,formno=formno)
...
def contactinfo():
    ...
    if request.method=='POST':
        ...
        query='''INSERT INTO '''+session['user']+''' VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'''
        cur.execute(query,(title,des,formno,'Name','text',0,'req',1,dl,limit))
        conn.commit()
        cur.execute(query,(title,des,formno,'Email','email',0,'req',2,dl,limit))
        conn.commit()
        cur.execute(query,(title,des,formno,'Address','text',0,'req',3,dl,limit))
        conn.commit()
        cur.execute(query,(title,des,formno,'Phone Number','number',0,'req',4,dl,limit))
        conn.commit()
        cur.execute(query,(title,des,formno,'Comments','text',0,'not req',5,dl,limit))
        conn.commit()
        return redirect(url_for('view',formno=formno)) 
...
def tshirt():
    ...
    if request.method=='POST':
        ...
        cur.execute(query,(title,des,formno,'Email','email',0,'req',2,dl,limit))
        conn.commit()
        cur.execute(query,(title,des,formno,'t-shirt size','radio','s','not req',3,dl,limit))
        conn.commit()
        cur.execute(query,(title,des,formno,'t-shirt size','radio','m','not req',3,dl,limit))
        conn.commit()
        cur.execute(query,(title,des,formno,'t-shirt size','radio','l','not req',3,dl,limit))
        conn.commit()
        cur.execute(query,(title,des,formno,'comments','text',0,'req',4,dl,limit))
        conn.commit()
        return redirect(url_for('view',formno=formno)) 
